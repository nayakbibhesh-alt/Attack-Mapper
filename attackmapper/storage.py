...
Two backends implement the same `Backend` protocol (same method
...
- `SQLiteStore` — the default for real usage (CLI/webapp). Persists to
  a single file (default `./attackmapper.db`, override with
  `ATTACKMAPPER_DB_PATH`) using only the stdlib `sqlite3` module, so a
  `attackmapper serve` session, or a `scan-nmap` + `infer` + `analyze`
  sequence of separate CLI invocations, all see the same data without
  needing Postgres or any extra install. This is what makes "scan a
  real target, close the UI, reopen it later" actually work end to
  end -- previously (Phase A-G) `_default_store` was always a
  fresh in-memory instance, silently discarded on every process exit.
...
def _build_default_store() -> Backend:
    ...
    persistent file, or "memory" for a process-local store that's
    ...
    "attackmapper.db" in the current working directory).
